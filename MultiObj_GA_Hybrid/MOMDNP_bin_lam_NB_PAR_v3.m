function [xopt,ND,funeval,lgen,fmin_hist] = MOMDNP_bin_lam_NB_PAR_v3(x0,ND,xopt...
    ,options,lb_con,ub_con,bits_con,lb_dis,ub_dis,bits_dis,fmin,flag,count,pair,Model,~,~,~,~,~,~,~,~,~,~)
...
%% ------------------------------------------------------------------------
new_gen = fgen;
x_des=zeros(pop_size,size(vlb,2));
funeval=0;
ter_crit=0;
ALL_obj_val=zeros(pop_size,2,max_gen+1);
ALL_x_des=zeros(pop_size,size(vlb,2),max_gen+1);
...
for ii=1:pop_size
    obj_val(ii,:)=objfunc(x0(ii,1:num_con),x0(ii,num_con+1:end),flag,count,pair,Model1);
    funeval=funeval+1;
end
% obj_val=[(fmin(1)+(fmax(1)-fmin(1))*rand(pop_size,1)),(fmin(2)+(fmax(2)-fmin(2))*rand(pop_size,1))];
fmin=frac_uto*[min(obj_val(:,1)),min(obj_val(:,2))];
% Header display
if PRINTING>=1,
    if ENCODED,
...
for generation = 1:max_gen+1,
    ...
    fmin_Cgen=frac_uto*[min(obj_val(:,1)),min(obj_val(:,2))];                                                                                                        
    if fmin_Cgen(1)<fmin(1)                                                                                                                                          
        fmin(1,1)=fmin_Cgen(1);                                                                                                                                     
    end                                                                                                                                                              
    if fmin_Cgen(2)<fmin(2)                                                                                                                                          
        fmin(1,2)=fmin_Cgen(2);                                                                                                                                      
    end     
...
%             goal=goalpointgen(fmin(1),fmax(2),fmax(1),fmin(2),aa(1),aa(2));
            ...
            [x_con,fval,~,exitflag,output]=fgoalattain(@(x_con) objfunc(x_con,x_dis,flag,count,pair,Model1),x0_con,gg,weight,[],[],[],[],lb_con,ub_con,@(x_con) constraints(x_con,x_dis,Model1),options);
            funeval=funeval+output.funcCount;
            g_check=constraint(x_con,x_dis,Model1);
...
for j = 1:2,
  ...
  [~,i_min] = min([obj_val(index,j);obj_val(index+1,j)]);
  %selected = i_min + [0:2:size(old_gen,1)-2];
  pot_I(count,:,j) = old_gen((index-1)+i_min,:);
  obj_pot_I(count,:,j)=obj_val((index-1)+i_min,:);
  count=count+1;
  end
end
% [pot_I,obj_pot_I]
...
pot_II(:,:,1)=pot_I(1:2*n,:,1);
pot_II(:,:,2)=pot_I(1:2*n,:,2);
...
count=1;
for j=(2*n)+1:(4*n)
    pot_II(count,:,3)=pot_I(j,:,1);
    pot_II(count+1,:,3)=pot_I(j,:,2);
...
new_gen=[pot_II(1:2*n,:,1);pot_II(1:2*n,:,2);pot_II(1:count-1,:,3)];
...
%Genetically tailored new generation to account for discrete technologies
forbid=[0 0 1];
...
for ii=1:pop_size
    ...
    if ii<=2*n %direction vector has slope 0
        goals(ii,1)=fmin(1);
        goals(ii,2)=obj_val(ii,2);
    elseif ii>2*n && ii<=4*n %direction vector has slope 90
        goals(ii,1)=obj_val(ii,1);
        goals(ii,2)=fmin(2);
    else
        xA_in=fmin(1);
        yA_in=obj_val(ii,2);
        ...
        f1=(obj_val(ii,1)-fmin(1))/fmin(1);
        f2=(obj_val(ii,2)-fmin(2))/fmin(2);
        x0=0;y0=0;
        xA=0;yA=f2;
        xB=f1;yB=0;
...
%         m=tan(pi/2-atan(((f2-y0)/(yA-y0))*((xB-x0)/(f1-x0)))); %Slope of the pointer
        ...
        gp_norm=[x0 y_int+y0;x_int+x0 y0];
