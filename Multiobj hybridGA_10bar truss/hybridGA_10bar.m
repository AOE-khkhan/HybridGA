function [xopt,ND,funeval,lgen,fmin_hist] = hybridGA_10bar(x0,ND,xopt...
...
for ii=1:pop_size     %%%%initial obj val-both functions & continuous & discrete
    obj_val(ii,:)=objfunc_10bar(x0(ii,1:num_con),x0(ii,num_con+1:end),flag,count); %%%%TRY BY NOT USING INITIAL FILE BY REPLACING x0(ii,1:num_variables) BY ORG. CODE
    funeval=funeval+1;
end
...
%             goal=goalpointgen(fmin(1),fmax(2),fmax(1),fmin(2),aa(1),aa(2));
            ...
            [x_con,fval,~,exitflag,output]=fgoalattain(@(x_con) objfunc_10bar(x_con,x_dis,flag,count),x0_con,gg,weight,[],[],[],[],lb_con,ub_con,@(x_con) constraints_10bar(x_con,x_dis),options);
            funeval=funeval+output.funcCount;
            g_check=constraints_10bar(x_con,x_dis); %%%%check? should be constraints...
