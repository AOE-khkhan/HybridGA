function [sigma,dL] = stress_3truss(A,E)
...
%x-y disp of each bar
dL1x = -L(1) + (-L(1)*cosd(theta1)-u(1));
dL2x = -L(2) + (-L(2)*cosd(theta2)-u(1));
dL3x = -L(3) + (-L(3)*cosd(theta3)-u(1));
dL1y = -L(1) + (-L(1)*sind(theta1)-u(2));
dL2y = -L(2) + (-L(2)*sind(theta2)-u(2));
dL3y = -L(3) + (-L(3)*sind(theta3)-u(2));

%final displacement in x-y directions
dLx = dL1x + dL2x + dL3x;
dLy = dL1y + dL2y + dL3y;
