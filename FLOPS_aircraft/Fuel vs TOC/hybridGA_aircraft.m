...
%             t_st=tic;
            ...
            w2=dK(i)*abs(gg(2));
            weight=[w1 w2];
            options=optimoptions(@fgoalattain,'Display','iter','FinDiffRelStep',1e-3);
            try
                [x_con,fval,~,exitflag,output]=fgoalattain(@(x_con) ...
                    objfunc_aircraft(x_con,x_dis,lb_con,ub_con,Filename),...
...
% Genetically tailored new generation 
% to account for discrete technologies for aircraft sizing
% Takes care of maximum 4 bits
% Work by Samarth Jain 03/14/17

%NUMBER OF DISCRETE CASES INPUT FOR EACH DISCRETE VARIABLE - dis_cases
