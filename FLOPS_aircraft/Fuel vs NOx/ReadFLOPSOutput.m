...
function [Outputs,nan_count,acdata_count,range_count] = ReadFLOPSOutput(filename)

TOC = [];BH = [];TD = [];LD = [];SSFOR = [];AMFOR = [];FUEL = []; DESRNG = [];RANGE = [];NOX = [];FARE = [];
fname = [filename '.out'];
fid = fopen(fname);
count = 0;
nlines = 0;
...
while ~feof(fid) && nan_count==0
    ...
    if ~isempty(tline)
        ...
        %% Read DESRNG
        if length(tline)>36 && strcmp(tline(1:36),'   DESIGN RANGE               DESRNG')==1% && count == 0
            %acdata_count = acdata_count + 1;
                [~,~,~,DESRNG,~,~] = textread(fname,'%s%s%s%f%s%s',1,'headerlines',nlines-1);
                %count = 1;   
        end
...
%         end
        ...
        if length(tline)>15 && strcmp(tline(1:15),'#OBJ/VAR/CONSTR')==1 && count == 0
            acdata_count = acdata_count + 1;
            try
                [FUEL,RANGE,~,TD,LD,AMFOR,SSFOR] = textread(fname,'%f%f%f%f%f%f%f',1,'headerlines',nlines+2);
                count = 1;
				if isnan(FUEL) == 1 || isnan(RANGE) == 1 || isnan(TD)==1 || isnan(LD) == 1
					nan_count=nan_count+1;
                    disp('\nFailed to give Fuel/RANGE/TD/LD\n')
					break
                end
                if RANGE < DESRNG
                    range_count=1;
